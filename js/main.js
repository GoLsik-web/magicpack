...
new Patron(world, at(site('town'), 0, -1, 20));   // стела покровителей у ворот столицы
...
const SHOTS = {
  ...
  patron: { pos: at(site('town'), -1, 4.5, 33), look: at(site('town'), -5, 3.6, 20) },
...
// ------------------------------------------------------------------ ступени орды
const MIND = [
  ['T0', 'Просто мертвецы', 'Идут напролом, по одному.'],
  ['T1', 'Слышат и приходят', 'Идут на шум. Кликни по земле — позовёшь сам.'],
  ['T2', 'Ждут своих', 'Не нападают, пока не соберётся стая.'],
  ['T3', 'Выбирают жертву', 'Бьют того, кто занят. Потеряв троих — бегут.'],
  ['T4', 'Давят числом', 'Обходят с флангов. Плетень не держит.'],
  ['T5', 'Тянут сквозь стену', 'Эндермен выдёргивает из-за укрытия. Камень ломают.'],
  ['T6', 'Прочёсывают район', 'Спрятался — ищут там, где видели.'],
  ['T7', 'Лунная ночь', 'Вся орда разом. Нужны стены и магия.'],
];
...
function setTier(i) {
  ...
  // заголовок выезжает, фраза проявляется по словам — коротко, без стены текста
  const words = (t, d0) => t.split(' ').map((w, k) => `<i style="--d:${(d0 + k * 0.06).toFixed(2)}s">${w}</i>`).join(' ');
  mindBody.innerHTML = `<b><em>${MIND[i][0]}</em>${words(MIND[i][1], 0)}</b><span>${words(MIND[i][2], 0.25)}</span>`;
